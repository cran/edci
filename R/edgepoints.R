edgepoints <- function(data,
  ...
  ep.r1 <- function (theta, x, y){
    sqrt(x^2+y^2)*ep.delta(x)*cos(ep.at(x,y)-theta)
  }
  
  ep.r2 <- function (theta, x, y){
    sqrt(x^2+y^2)*ep.delta(x)*sin(ep.at(x,y)-theta)
  }
    ...
    for (i in ((-env):env)){           
      for(j in ((-env):env)){        
        for (k in (0:(asteps-1))){   
          ...
          x=ep.r1(theta,i/nrow(data),j/ncol(data))/h1n
          y=ep.r2(theta,i/nrow(data),j/ncol(data))/h2n
...
eplist <- function(data,maxval,test=FALSE,xc=NULL,yc=NULL){
  ...
  result <- cbind(xc[(o-1) %%  nrow(data[[1]]) +1],
                  yc[(o-1) %/% nrow(data[[1]]) +1],
                  data[[2]][o])
